...
/**
 * Created on 30/10/2019 Package me.jumba.bitdefender.checks.movement.speed
 */
    ...
    public void onPacket(PacketEvent e) {
        if (isPacketMovement(e.getType())) {
            ...
            if (user.getNewFrom() != null && user.newTo != null) {
                ...
                if (fix >= 1.04 && !onGround && !lastOnGround && user.getNewTo().getY() != user.getNewFrom().getY()) {
                    if (user.speedAVerbose++ > 1) {
                        flag(user);
                    }
                } else user.speedAVerbose -= user.speedAVerbose > 0 ? 1 : 0;
            }
        }
    }
}
