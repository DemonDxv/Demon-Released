...
import dev.demon.events.exceptions.ListenParamaterException;
...
public class EventManager {
    ...
    public void registerListener(Method method, BitDefenderListener listener, Plugin plugin) throws ListenParamaterException {
        if(method.getParameterTypes().length == 1) {
            if(method.getParameterTypes()[0].getSuperclass().equals(BitDefenderEvent.class)) {
                ...
                throw new ListenParamaterException("Method " + method.getDeclaringClass().getName() + "#" + method.getName() + "'s paramater: " + method.getParameterTypes()[0].getName() + " is not an instanceof " + BitDefenderEvent.class.getSimpleName() + "!");
            }
        } else {
    ...
    public void unregisterListener(BitDefenderListener listener) {
        listenerMethods.stream().filter(lm -> lm.listener.equals(listener)).forEach(listenerMethods::remove);
    }

    public void registerListeners(BitDefenderListener listener, Plugin plugin) {
    ...
    public void callEvent(BitDefenderEvent event) {
    ...
    public void callEventAsync(BitDefenderEvent event) {
        Demon.getInstance().getExecutorService().execute(() -> callEvent(event));
    }
}